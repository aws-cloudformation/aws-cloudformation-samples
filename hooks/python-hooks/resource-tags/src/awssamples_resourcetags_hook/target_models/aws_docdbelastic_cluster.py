# DO NOT modify this file by hand, changes will be overwritten
from dataclasses import dataclass

from cloudformation_cli_python_lib.interface import BaseModel
from cloudformation_cli_python_lib.recast import recast_object
from cloudformation_cli_python_lib.utils import deserialize_list

import sys
from inspect import getmembers, isclass
from typing import (
    AbstractSet,
    Any,
    Generic,
    Mapping,
    MutableMapping,
    Optional,
    Sequence,
    Type,
    TypeVar,
)

T = TypeVar("T")


def set_or_none(value: Optional[Sequence[T]]) -> Optional[AbstractSet[T]]:
    if value:
        return set(value)
    return None


@dataclass
class AwsDocdbelasticCluster(BaseModel):
    ClusterName: Optional[str]
    ClusterArn: Optional[str]
    ClusterEndpoint: Optional[str]
    AdminUserName: Optional[str]
    AdminUserPassword: Optional[str]
    ShardCapacity: Optional[int]
    ShardCount: Optional[int]
    VpcSecurityGroupIds: Optional[Sequence[str]]
    SubnetIds: Optional[Sequence[str]]
    PreferredMaintenanceWindow: Optional[str]
    KmsKeyId: Optional[str]
    Tags: Optional[Any]
    AuthType: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_AwsDocdbelasticCluster"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_AwsDocdbelasticCluster"]:
        if not json_data:
            return None
        dataclasses = {n: o for n, o in getmembers(sys.modules[__name__]) if isclass(o)}
        recast_object(cls, json_data, dataclasses)
        return cls(
            ClusterName=json_data.get("ClusterName"),
            ClusterArn=json_data.get("ClusterArn"),
            ClusterEndpoint=json_data.get("ClusterEndpoint"),
            AdminUserName=json_data.get("AdminUserName"),
            AdminUserPassword=json_data.get("AdminUserPassword"),
            ShardCapacity=json_data.get("ShardCapacity"),
            ShardCount=json_data.get("ShardCount"),
            VpcSecurityGroupIds=json_data.get("VpcSecurityGroupIds"),
            SubnetIds=json_data.get("SubnetIds"),
            PreferredMaintenanceWindow=json_data.get("PreferredMaintenanceWindow"),
            KmsKeyId=json_data.get("KmsKeyId"),
            Tags=json_data.get("Tags"),
            AuthType=json_data.get("AuthType"),
        )


# work around possible type aliasing issues when variable has same name as a model
_AwsDocdbelasticCluster = AwsDocdbelasticCluster


@dataclass
class Tag(BaseModel):
    Key: Optional[str]
    Value: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_Tag"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_Tag"]:
        if not json_data:
            return None
        return cls(
            Key=json_data.get("Key"),
            Value=json_data.get("Value"),
        )


# work around possible type aliasing issues when variable has same name as a model
_Tag = Tag


