# DO NOT modify this file by hand, changes will be overwritten
from dataclasses import dataclass

from cloudformation_cli_python_lib.interface import BaseModel
from cloudformation_cli_python_lib.recast import recast_object
from cloudformation_cli_python_lib.utils import deserialize_list

import sys
from inspect import getmembers, isclass
from typing import (
    AbstractSet,
    Any,
    Generic,
    Mapping,
    MutableMapping,
    Optional,
    Sequence,
    Type,
    TypeVar,
)

T = TypeVar("T")


def set_or_none(value: Optional[Sequence[T]]) -> Optional[AbstractSet[T]]:
    if value:
        return set(value)
    return None


@dataclass
class AwsWorkspaceswebPortal(BaseModel):
    AdditionalEncryptionContext: Optional[MutableMapping[str, str]]
    AuthenticationType: Optional[str]
    BrowserSettingsArn: Optional[str]
    BrowserType: Optional[str]
    CreationDate: Optional[str]
    CustomerManagedKey: Optional[str]
    DisplayName: Optional[str]
    IpAccessSettingsArn: Optional[str]
    NetworkSettingsArn: Optional[str]
    PortalArn: Optional[str]
    PortalEndpoint: Optional[str]
    PortalStatus: Optional[str]
    RendererType: Optional[str]
    ServiceProviderSamlMetadata: Optional[str]
    StatusReason: Optional[str]
    Tags: Optional[Any]
    TrustStoreArn: Optional[str]
    UserAccessLoggingSettingsArn: Optional[str]
    UserSettingsArn: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_AwsWorkspaceswebPortal"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_AwsWorkspaceswebPortal"]:
        if not json_data:
            return None
        dataclasses = {n: o for n, o in getmembers(sys.modules[__name__]) if isclass(o)}
        recast_object(cls, json_data, dataclasses)
        return cls(
            AdditionalEncryptionContext=json_data.get("AdditionalEncryptionContext"),
            AuthenticationType=json_data.get("AuthenticationType"),
            BrowserSettingsArn=json_data.get("BrowserSettingsArn"),
            BrowserType=json_data.get("BrowserType"),
            CreationDate=json_data.get("CreationDate"),
            CustomerManagedKey=json_data.get("CustomerManagedKey"),
            DisplayName=json_data.get("DisplayName"),
            IpAccessSettingsArn=json_data.get("IpAccessSettingsArn"),
            NetworkSettingsArn=json_data.get("NetworkSettingsArn"),
            PortalArn=json_data.get("PortalArn"),
            PortalEndpoint=json_data.get("PortalEndpoint"),
            PortalStatus=json_data.get("PortalStatus"),
            RendererType=json_data.get("RendererType"),
            ServiceProviderSamlMetadata=json_data.get("ServiceProviderSamlMetadata"),
            StatusReason=json_data.get("StatusReason"),
            Tags=json_data.get("Tags"),
            TrustStoreArn=json_data.get("TrustStoreArn"),
            UserAccessLoggingSettingsArn=json_data.get("UserAccessLoggingSettingsArn"),
            UserSettingsArn=json_data.get("UserSettingsArn"),
        )


# work around possible type aliasing issues when variable has same name as a model
_AwsWorkspaceswebPortal = AwsWorkspaceswebPortal


@dataclass
class Tag(BaseModel):
    Key: Optional[str]
    Value: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_Tag"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_Tag"]:
        if not json_data:
            return None
        return cls(
            Key=json_data.get("Key"),
            Value=json_data.get("Value"),
        )


# work around possible type aliasing issues when variable has same name as a model
_Tag = Tag


