This AWS CloudFormation Hook validates that encryption is enabled when creating or updating a an AWS RDS DBInstance. This hook validates that the  `StorageEncrypted` property of the`AWSSamples::RdsEncrypt::Hook` resource is not set or is not`true`.
